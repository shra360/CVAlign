...
import { getApplications, updateApplication, deleteApplication, getJobs, getUserProfile, getRecruiterCutoff } from '../lib/firebase';
...
export function ApplicationsPage({ jobId: propJobId }: { jobId?: string } = {}) {
  ...
  // If a jobId prop is provided, set the filter on mount
  useEffect(() => {
    if (propJobId) {
      setFilters(prev => ({ ...prev, jobId: propJobId }));
    }
  }, [propJobId]);
  ...
  const loadApplications = async () => {
    try {
      ...
      // Attach candidate profile photos
      const appsWithPhotos = await Promise.all(apps.map(async (app: any) => {
        if (app.candidateId) {
          const candidateProfile = await getUserProfile(app.candidateId);
          return {
            ...app,
            candidateProfileImageUrl: candidateProfile?.profileImageUrl || candidateProfile?.photoURL || '',
          };
        }
        return app;
      }));
      setApplications(appsWithPhotos);
    } catch (error) {
  ...
  // For candidates: load only their own applications
  const loadCandidateApplications = async () => {
    try {
      setLoading(true);
      const apps = await getApplications({ candidateId: userProfile?.id });
      // Attach candidate profile photos
      const appsWithPhotos = await Promise.all(apps.map(async (app: any) => {
        if (app.candidateId) {
          const candidateProfile = await getUserProfile(app.candidateId);
  ...
  const loadCutoffScore = async () => {
    try {
      if (userProfile?.role === 'recruiter') {
        const cutoff = await getRecruiterCutoff(userProfile.id);
        setCutoffScore(cutoff);
      }
    } catch (error) {
      console.error('Error loading cutoff score:', error);
    }
  };

  const loadJobs = async () => {
    try {
      // For candidates, load all jobs; for recruiters, load only their jobs
      const jobsData = userProfile?.role === 'candidate'
        ? await getJobs() // no filter
        : await getJobs({ recruiterId: userProfile?.id });
      setJobs(jobsData);
    } catch (error) {
  ...
  const filteredApplications = sortedApplications.filter(app => {
    ...
    // Different search logic for candidates vs recruiters
    let matchesSearch = true;
    if (filters.search.trim() !== '') {
      if (userProfile?.role === 'candidate') {
        // For candidates, search by job title and recruiter name
        const jobTitle = getJobTitle(app).toLowerCase();
        const recruiterName = getRecruiterName(app).toLowerCase();
        matchesSearch = jobTitle.includes(filters.search.toLowerCase()) ||
                       recruiterName.includes(filters.search.toLowerCase());
      } else {
        // For recruiters, search by candidate name and email
        matchesSearch = app.candidateName.toLowerCase().includes(filters.search.toLowerCase()) ||
  ...
  const getScoreColor = (score: number) => {
    if (userProfile?.role === 'recruiter') {
      // For recruiters, use cutoff score
      if (score >= cutoffScore) return 'text-success-600';
      return 'text-error-600';
    } else {
      // For candidates, use fixed thresholds
  ...
  // For candidates: sort applications by date (latest first), then by job title
  let candidateSortedApplications = filteredApplications;
  if (userProfile?.role === 'candidate') {
    candidateSortedApplications = [...filteredApplications].sort((a, b) => {
      ...
      // If dates are the same, sort by job title
      const jobA = getJobTitle(a).toLowerCase();
      const jobB = getJobTitle(b).toLowerCase();
      if (jobA < jobB) return -1;
      if (jobA > jobB) return 1;
      
      // If job titles are the same, sort by score descending
      if (typeof b.score === 'number' && typeof a.score === 'number') {
        return b.score - a.score;
      }
      if (typeof b.score === 'number') return 1;
  ...
  // For candidate job filter dropdown: only jobs they've applied to
  const appliedJobIds = Array.from(new Set(applications.map((app: any) => app.jobId)));
  const appliedJobs = jobs.filter((job: any) => appliedJobIds.includes(job.id));
  const candidateJobOptions = [
    { value: 'all', label: 'All Jobs' },
    ...appliedJobs.map((job: any) => ({ value: job.id, label: job.title }))
  ];
  const recruiterJobOptions = [
    { value: 'all', label: 'All Jobs' },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-secondary-600">You must be logged in to view applications.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder={userProfile?.role === 'candidate' ? "Search by job title or recruiter name" : "Search by name or email"}
                className="pl-10"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <Select
              value={filters.jobId}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFilters(prev => ({ ...prev, jobId: e.target.value }))}
              options={userProfile?.role === 'candidate' ? candidateJobOptions : recruiterJobOptions}
            />
          </div>
        </div>
      </Card>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {candidateSortedApplications.length === 0 ? (
            <Card>
              <div className="text-center py-8">
                ...
                <p className="mt-1 text-sm text-secondary-500">You have not applied to any jobs yet.</p>
              </div>
            </Card>
          ) : (
            candidateSortedApplications.map((application) => (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-secondary-900">{getRecruiterName(application)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-sm text-secondary-600">CGPA</p>
                      <p className="font-medium text-secondary-900">
                        {application.cgpa ? `${application.cgpa}` : 'Not provided'}
                      </p>
                    </div>
                    <div>
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-3">
                    <div className="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center overflow-hidden">
                      {application.candidateProfileImageUrl ? (
                        <img src={application.candidateProfileImageUrl} alt={application.candidateName} className="w-full h-full object-cover" />
                      ) : (
                ...
                <div className="flex flex-col items-end space-y-3">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleOpenChat(application.recruiterId)}
                    >
                      <MessageIcon />
                  ...
                  <Select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleStatusUpdate(application.id, e.target.value)}
                    options={[
                      { value: 'pending', label: 'Pending' },
                      { value: 'reviewed', label: 'Reviewed' },
                      { value: 'shortlisted', label: 'Shortlisted' },
