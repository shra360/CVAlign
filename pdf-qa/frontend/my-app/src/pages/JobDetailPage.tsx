...
export function JobDetailPage() {
  ...
  const loadJob = async () => {
    try {
      ...
      // Extract text from JD PDF if available
      if ((jobData as any)?.jdFileUrl) {
        await extractJdText((jobData as any).jdFileUrl);
      }
    } catch (error) {
  ...
  const extractJdText = async (pdfUrl: string) => {
    try {
      ...
      const pdfFile = new File([pdfBlob], 'job_description.pdf', { type: 'application/pdf' });
      
      formData.append('pdf_file', pdfFile);
      
      // Send to the text extraction API
      const extractResponse = await fetch('http://localhost:8501/api/extract-text', {
        method: 'POST',
        body: formData
      });
      ...
      setJdText('Unable to load job description. Please contact the recruiter for details.');
    } finally {
  ...
  const handleCgpaChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === '' || /^\d*\.?\d{0,2}$/.test(value)) {
      setApplicationData(prev => ({ ...prev, cgpa: value }));
      // Clear CGPA error when user starts typing
      setCgpaError('');
      
      // Check if entered CGPA is below minimum requirement (for real-time feedback)
      if (value && job?.minCGPA) {
        const cgpaValue = parseFloat(value);
        if (!isNaN(cgpaValue) && cgpaValue < job.minCGPA) {
          setCgpaError(`Your CGPA (${value}) is below the minimum requirement (${job.minCGPA})`);
        }
      }
    }
  };
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    // Validate CGPA range
    if (applicationData.cgpa) {
      const cgpaValue = parseFloat(applicationData.cgpa);
      ...
      const decimalPlaces = applicationData.cgpa.split('.')[1]?.length || 0;
      if (decimalPlaces > 2) {
      ...
      // Check if candidate's CGPA meets the minimum requirement
      if (job.minCGPA && cgpaValue < job.minCGPA) {
        setCgpaError(`Your CGPA (${applicationData.cgpa}) is below the minimum requirement (${job.minCGPA})`);
        toast.error(`Your CGPA is below the minimum requirement of ${job.minCGPA}`);
        return;
      }
    } else {
    ...
    try {
      ...
      const application = {
        ...
        cgpa: applicationData.cgpa ? parseFloat(applicationData.cgpa) : undefined,
      ...
      setApplicationData({ cvFile: null, coverLetter: '', cgpa: '' });
      setCvFileName('');
      setCgpaError('');
  ...
  const formatSalary = (salary: any) => {
    ...
    const currencySymbol = getCurrencySymbol(salary.currency);
    return `${currencySymbol} ${salary.min.toLocaleString()} - ${salary.max.toLocaleString()}`;
  };
              ...
              <div className="flex items-center text-sm text-secondary-600">
                <div className="mr-2">
                  <span className="text-lg font-bold text-secondary-600">
                    {job.salary?.currency ? getCurrencySymbol(job.salary.currency) : '$'}
                  </span>
                </div>
          ...
          <Card>
            ...
            <div className="prose max-w-none">
              {extractingJd ? (
                <div className="text-center py-8">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-600 mx-auto"></div>
                  <p className="mt-2 text-secondary-600">Extracting job description from PDF...</p>
                </div>
              ) : jdText ? (
                <div className="text-secondary-700 whitespace-pre-wrap leading-relaxed">
                  {jdText}
                </div>
              ) : job?.jdFileUrl ? (
                <div className="text-center py-8">
                  <p className="text-secondary-600 mb-4">Unable to extract text from PDF.</p>
                  <Button
                    onClick={() => window.open(job.jdFileUrl, '_blank')}
                    className="bg-blue-600 text-white hover:bg-blue-700"
                  >
                    <FileTextIcon />
                    <span className="ml-2">View Original PDF</span>
                  </Button>
                </div>
              ) : (
                <p className="text-secondary-600 italic">No job description available.</p>
              )}
            </div>
          </Card>
        </div>
          ...
          {showApplicationForm && userProfile?.role === 'candidate' && (
            <Card>
              ...
              <form onSubmit={handleApply} className="space-y-4">
                ...
                {/* CGPA */}
                <div>
                  <label htmlFor="cgpa" className="block text-sm font-medium text-secondary-700 mb-1">
                    CGPA <span className="text-error-600">*</span>
                  </label>
                  <Input
                    id="cgpa"
                    type="text"
                  ...
                  {job.minCGPA && (
                    <p className="mt-1 text-sm text-warning-600 font-medium">
                      ⚠️ Minimum CGPA required: <strong>{job.minCGPA}</strong>
                    </p>
                  )}
                </div>
                ...
                <div className="flex space-x-3">
                  ...
                  <Button
                    type="submit"
                    disabled={applying || !applicationData.cvFile || cgpaError}
                    className="flex-1"
                  >
