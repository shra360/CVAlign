...
export function ChatWindowPage() {
        ...
        if (chatDoc.exists()) {
          ...
          if (otherParticipantId) {
            ...
            if (otherUserDoc.exists()) {
              ...
              console.log('Fetched chat partner data:', otherUserData);
              setChatPartner({
                name: otherUserData.displayName || otherUserData.name || 'Unknown User',
                avatar: otherUserData.profileImageUrl || otherUserData.photoURL || '',
                id: otherUserDoc.id,
              });
              console.log('Set chat partner avatar to:', otherUserData.profileImageUrl || otherUserData.photoURL || '');
            } else {
  ...
  const handleProfileClick = () => {
    if (chatPartner?.id) {
      navigate(`/profiles/${chatPartner.id}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-secondary-50 flex flex-col items-center">
      <div className="w-full max-w-lg flex flex-col flex-1 bg-white shadow-lg rounded-lg overflow-hidden my-4">
        {/* Header */}
        <div className="border-b border-secondary-200 p-4 flex items-center space-x-3">
          <button
            ...
            <div className="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center overflow-hidden">
              {chatPartner?.avatar ? (
                <img 
                  ...
                  onError={(e) => {
                    console.log('Failed to load profile image:', chatPartner.avatar);
                    e.currentTarget.style.display = 'none';
                    e.currentTarget.nextElementSibling?.classList.remove('hidden');
                  }}
                />
              ) : null}
              <span className={`text-primary-600 font-semibold ${chatPartner?.avatar ? 'hidden' : ''}`}>
        ...
        <div
          ...
          className="flex-1 p-4 overflow-y-auto space-y-4 bg-secondary-50"
        >
