...
import { getUsers, getApplications, getRecruiterCutoff } from '../lib/firebase';
...
const Badge = ({ children, variant = 'default', className = '' }: { children: React.ReactNode, variant?: BadgeVariant, className?: string }) => {
  ...
  const safeVariant: BadgeVariant = (['default','success','warning','error','primary'].includes(variant) ? variant : 'default') as BadgeVariant;
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${variants[safeVariant]} ${className}`}>
      {children}
    </span>
  );
};
...
export function CandidateSearchPage() {
  ...
  const handleContactCandidate = async (candidateId: string) => {
    if (!userProfile || !candidateId || initiatingChat) return;
    setInitiatingChat(true);
    try {
      // Find candidate user object
      const candidate = filteredCandidates.find(c => c.id === candidateId);
      if (!candidate) return;
      // Chat logic (same as Layout)
      const user = userProfile;
      const chatsRef = collection(db, 'users', user.id, 'chats');
      const existingChatQuery = query(chatsRef, where('otherUserId', '==', candidateId));
      ...
      const newChatId = [user.id, candidateId].sort().join('_');
      const currentUserChatData = {
        otherUserId: candidateId,
        participants: [user.id, candidateId],
      ...
      try {
        await setDoc(
          doc(db, 'users', user.id, 'chats', newChatId),
          currentUserChatData
        );
      } catch (error) {
        throw error;
      ...
      try {
        await setDoc(
          doc(db, 'users', candidateId, 'chats', newChatId),
          otherUserChatData
        );
      } catch (error) {
        try {
          await deleteDoc(doc(db, 'users', user.id, 'chats', newChatId));
        } catch {}
        throw error;
      }
  ...
  const getRatingColor = (rating: number) => {
    if (rating >= cutoffScore) return 'text-success-600';
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between">
                  ...
                  {candidate.rating && (
                    <div className="flex items-center space-x-1">
                      <StarIcon />
                      <span className={`text-sm font-medium ${getRatingColor(candidate.rating)}`}>{candidate.rating}%</span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  {candidate.companyName && (
                    <div>
                      <span className="text-sm font-medium text-secondary-700">Company:</span>
                      <span className="text-sm text-secondary-600 ml-1">{candidate.companyName}</span>
                    </div>
                  )}
      ...
      {showProfileModal && selectedProfile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-2xl w-full relative">
            <button className="absolute top-2 right-2 text-secondary-500 hover:text-secondary-900" onClick={() => setShowProfileModal(false)}>&times;</button>
