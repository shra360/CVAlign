...
// Set the user's shortlistedResetAt field
export const setShortlistedResetAt = async (userId: string, date: Date) => {
  await updateDoc(doc(db, 'users', userId), { shortlistedResetAt: date, updatedAt: new Date() });
};

// Get the user's shortlistedResetAt field
export const getShortlistedResetAt = async (userId: string) => {
  const docRef = doc(db, 'users', userId);
  const docSnap = await getDoc(docRef);
  if (docSnap.exists()) {
    return docSnap.data().shortlistedResetAt ? new Date(docSnap.data().shortlistedResetAt.seconds ? docSnap.data().shortlistedResetAt.seconds * 1000 : docSnap.data().shortlistedResetAt) : null;
  }
  return null;
}; 
