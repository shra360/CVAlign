...
export function Sidebar() {
  ...
  const getNavItemClass = (path: string, disabled = false) => {
    if (disabled) {
      return 'flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors duration-200 text-secondary-400 cursor-not-allowed opacity-50';
    }
  ...
  const recruiterNavItems = [
    ...
    { 
      ...
      label: 'Post Job', 
      icon: <PlusIcon />,
      disabled: !userProfile?.companyName || !userProfile?.designation
    },
    ...
    <div className="flex flex-col w-64 bg-white border-r border-secondary-200">
      <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
        <div className="flex items-center flex-shrink-0 px-4 mb-6">
          ...
          <h1 className="text-2xl font-extrabold text-primary-700 tracking-tight">HireSync</h1>
        </div>
          ...
          {navItems.map((item) => {
            ...
            if (isDisabled) {
              return (
                <div
                  key={item.path}
                  className={getNavItemClass(item.path, true)}
                  title="Complete your company information in profile to post jobs"
                >
                  {itemContent}
                </div>
              );
